...
export default function ReturnHistory() {
  ...
  const filteredReturns = (returns || [])
    ...
    .sort((a, b) => {
      ...
      switch (col) {
        ...
        case "note": aVal = extractNote(a); bVal = extractNote(b); break;
  ...
  const exportToExcel = useCallback(() => {
    try {
      ...
      const colWidths = [
        {wch:20}, {wch:20}, {wch:25}, {wch:18}, {wch:18}, {wch:10},
        {wch:12}, {wch:15}, {wch:15}, {wch:30}, {wch:50} 
      ];
      ws['!cols'] = colWidths;
  ...
  const handlePrint = useCallback((returnItem) => {
    ...
    const noteHtml = displayNote
      ? `<div style="background:#eff6ff;padding:15px;border-radius:10px;margin-bottom:25px;border-left:4px solid #3b82f6;">
           <strong style="color:#1e40af;">📝 Note:</strong> <span style="color:#1e3a8a;">${escapeHtml(displayNote)}</span>
         </div>`
      : "";
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body { font-family: 'Segoe UI', 'Poppins', Arial, sans-serif; background: #f0f2f5; margin: 0; padding: 20px; }
    .print-container { max-width: 1000px; margin: 0 auto; background: white; border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); overflow: hidden; }
    .print-content { padding: 40px; }
    @media print { body { background: white; padding: 0; } .print-container { box-shadow: none; border-radius: 0; } .print-content { padding: 20px; } }
    .header { text-align: center; margin-bottom: 35px; border-bottom: 3px solid #7c3aed; padding-bottom: 25px; }
    .logo { height: 80px; object-fit: contain; margin-bottom: 15px; }
    .company-name { font-size: 32px; font-weight: bold; color: #7c3aed; margin: 0; letter-spacing: 1px; }
    .bill-banner { background: linear-gradient(135deg, #2c3e50 0%, #1a252f 100%); color: white; padding: 20px; border-radius: 10px; margin-bottom: 25px; display: flex; justify-content: space-between; }
    .status-badge { background: ${statusBg}; color: ${statusColor}; padding: 5px 15px; border-radius: 25px; display: inline-block; font-weight: 600; }
    .items-table { width: 100%; border-collapse: collapse; margin-bottom: 25px; border-radius: 12px; overflow: hidden; }
    .items-table th { background: #7c3aed; color: white; padding: 14px; font-weight: 600; font-size: 13px; }
    .items-table td { padding: 12px; border: 1px solid #e5e7eb; }
    .total-row { background: #f3f4f6; font-weight: bold; }
    .total-amount { color: #059669; font-size: 18px; font-weight: bold; }
    .signatures { margin-top: 50px; border-top: 2px dashed #9ca3af; padding-top: 30px; display: flex; justify-content: space-between; gap: 40px; }
    .signature-line { flex: 1; text-align: center; }
    .signature-dash { border-top: 2px solid #4b5563; width: 80%; margin: 40px auto 10px; padding-top: 10px; }
...
<body>
  <div class="print-container">
    <div class="print-content">
      <div class="header" style="display: flex; align-items: center; gap: 15px; width: 100%; padding: 10px 0;">
        ...
        <div class="contact-info" style="flex: 1; text-align: right; font-size: 0.8em;">
          <div class="company-info">
            ...
            <p style="margin: 2px 0; color: #6b7280">📞 +964 772 533 5252 | +964 751 741 2241</p>
            <p style="margin: 2px 0; color: #6b7280">📍 سلێمانی بەرامبەر تاوەری تەندروستی سمارت</p>
          </div>
        </div>
      </div>
      <div class="bill-banner">
        <div style="color: white;">
          <p style="margin: 8px 0;"><strong>📄 Return #:</strong> ${returnItem.returnBillNumber}</p>
          <p style="margin: 8px 0;"><strong>🏪 Pharmacy:</strong> ${escapeHtml(pharmacyNameDisplay)}</p>
          <p style="margin: 8px 0;"><strong>📅 Date:</strong> ${formatDate(returnItem.returnDate)}</p>
        </div>
        <div style="text-align: right; color: white;">
          <p style="margin: 8px 0;"><strong>🏥 Pharmacy Return #:</strong> ${escapeHtml(pharmacyReturnNumberDisplay)}</p>
          <p style="margin: 8px 0;"><strong>🧾 Original Bill:</strong> ${returnItem.billNumber}</p>
          <p style="margin: 8px 0;"><strong>🔖 Status:</strong> <span class="status-badge">${returnItem.paymentStatus}</span></p>
        </div>
      </div>
      <table class="items-table">
        <thead>
          <tr>
            <th>#</th><th>Item Name</th><th>Barcode</th><th>Quantity</th><th>Unit Price</th><th>Total</th>
          </tr>
        </thead>
        <tbody>
        ...
        <tfoot>
          <tr class="total-row">
            ...
            <td style="padding:14px;text-align:right;" class="total-amount">${formatCurrency(returnItem.totalReturnAmount, currency)}</td>
          </tr>
        </tfoot>
      </table>
      ${noteHtml}
      ...
      <div class="footer"><p class="print-date">Printed on: ${new Date().toLocaleString()}</p></div>
    </div>
  </div>
</body>
</html>`;
  ...
  const styles = {
    ...
    badge: { display: "inline-flex", alignItems: "center", padding: "3px 10px", borderRadius: "6px", fontSize: "14px", fontWeight: "500", whiteSpace: "nowrap" },
    ...
    btn: { display: "inline-flex", alignItems: "center", gap: "6px", padding: "8px 16px", borderRadius: "8px", fontSize: "14px", fontWeight: "500", cursor: "pointer", border: "none", transition: "all 0.2s", whiteSpace: "nowrap" },
  ...
  if (isLoading && returns.length === 0) {
    return (
      <div className="page-wrapper">
        <div className="page-inner mobile-padded">
          <h1 style={styles.headerTitle}>Return History</h1>
          <div className="card-wrapper" style={{ textAlign: "center", padding: "40px" }}>Loading return history...</div>
        </div>
      </div>
    );
  }
    ...
    <>
      <style>{`
        .page-wrapper {
          ...
          padding: 1.5rem;
          box-sizing: border-box;
          font-family: 'NRT-Reg', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        }
        .page-inner {
          width: 100%;
          max-width: 100%;
        ...
        .mobile-padded {
          /* Inherits spacing safely on desktop */
        }
      ...
      <div className="page-wrapper">
        <div className="page-inner">
            ...
            <div style={styles.cardHeader}>
              <div style={styles.flexBetween}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <button onClick={exportToExcel} disabled={isExporting || filteredReturns.length === 0} style={{ ...styles.btn, ...styles.btnSuccess, ...styles.btnSmall }}>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                    ...
                    return (
                      <React.Fragment key={returnItem.id}>
                        ...
                        >
                          <td style={styles.tableCell}>
                            ...
                            <button
                              ...
                              onClick={e => { e.stopPropagation(); handlePrint(returnItem); }}
                            >
                          ...
                          <td style={styles.tableCell}>
                            <div style={{ maxWidth: "150px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={displayNote}>
                              {displayNote || "—"}
                            </div>
                          </td>
                          <td style={styles.tableCell}>
                        ...
                        {selectedReturn?.id === returnItem.id && (
                          <tr>
                            <td colSpan="10" style={{ padding: "20px", backgroundColor: "#f9fafb" }}>
                  ...
                  {filteredReturns.length === 0 && (
                    <tr>
                      <td colSpan="10" style={{ padding: "60px", textAlign: "center", color: "#6b7280" }}>
          ...
          {selectedPharmacy?.id && (
            <div className="card-wrapper" ref={editSectionRef}>
              <div style={styles.cardHeader}>
                                                    ...
                                                    {returnItems.map((item, idx) => {
                                                      ...
                                                      return (
                                                        <tr key={idx} style={{ borderBottom: "1px solid #e5e7eb" }}>
                                                          ...
                                                          <td style={{ padding: "8px", textAlign: "right", fontWeight: "bold", color: "#059669" }}>{formatCurrency(itemTotal, item.currency)}</td>
                                                          <td style={{ padding: "8px", textAlign: "center" }}>
                            ...
                            {returnItems.map((item, idx) => {
                              ...
                              return (
                                <tr key={idx} style={{ borderBottom: "1px solid #e5e7eb" }}>
                                  ...
                                  <td style={{ padding: "12px", textAlign: "right", fontWeight: "bold", color: "#059669" }}>{formatCurrency(itemTotal, item.currency)}</td>
                                  <td style={{ padding: "12px", textAlign: "center" }}>
                                    <button style={{ ...styles.btn, ...styles.btnDanger, padding: "4px 8px", fontSize: "14px" }} onClick={() => handleDeleteReturnItemFromList(idx)}>
                                      <FaTrash size={10} /> Remove
                                    </button>
