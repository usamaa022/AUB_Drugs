  ...
  try {
    ...
    if (status === "received" && receivedItems.length > 0) {
      for (const item of receivedItems) {
        ...
        // 1. Add arrived items to destination branch store
        ...
        // 2. Record missing quantity with base prices and notes preserved
...
// ============================================================
// MISSING TRANSPORT ITEMS RESOLUTION
// ============================================================
...
export async function resolveMissingTransportItem(missingItemId, targetBranch, userUid, foundQuantity = null) {
  try {
    ...
    // Resolve full quantity if null, or partial quantity if specified
    ...
    // Find existing item in target store
    const storeSnap = await getDocs(
      query(
        collection(db, STORE_ITEMS_COLLECTION),
        where("barcode", "==", missingData.barcode),
    ...
    for (const docSnap of storeSnap.docs) {
      const si = docSnap.data();
      const sameExpire = toFirestoreTimestamp(si.expireDate)?.isEqual(normalizedExpireDate) || 
        (!si.expireDate && !normalizedExpireDate);
    ...
    if (matchingDoc) {
      // Merge into existing row with preserved base price
