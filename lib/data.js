...
export async function sendTransport(fromBranch, toBranch, items, senderId, sendDate, notes) {
  try {
    ...
    for (const item of items) {
      ...
      const availableQuantity = matched.reduce((sum, doc) => sum + (Number(doc.data().quantity) || 0), 0);
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (totalAvailable < returnQty) {
        throw new Error(`Cannot return ${returnQty} of "${item.name}". Only ${totalAvailable} currently in store.`);
      }
      ...
      processedItems.push({
        ...
        restoredAllocations: restoreAllocations,
      });
    }
...
export async function updateBoughtReturnBill(returnDocId, updatedItems, note) {
  try {
    ...
    const oldItems = Array.isArray(returnDocData.items) && returnDocData.items.length > 0
      ? returnDocData.items
    ...
    for (const newItem of updatedItems) {
      ...
      if (newQty > maxReturnable) {
        throw new Error(
          `Cannot set return quantity to ${newQty} for "${newItem.name}". Only ${maxReturnable} units remain eligible from bill #${itemBillNumber}.`
        );
      }
      ...
      if (delta > 0) {
        ...
        if (totalAvailable < delta) {
          throw new Error(`Cannot increase return quantity of "${newItem.name}" by ${delta}. Only ${totalAvailable} units currently in store.`);
        }
...
export async function deleteBoughtReturnItem(returnDocId, barcode) {
  try {
    ...
    const itemsArray = Array.isArray(returnDocData.items) && returnDocData.items.length > 0
...
export async function deleteReturnBillAndRestoreToSale(returnId) {
  try {
    ...
    const itemsToReverse = Array.isArray(returnData.items) && returnData.items.length > 0
      ? returnData.items
      : returnData.barcode ? [returnData] : [];
...
// 1. FIXED getAllReturns (Removed the / 1500 bug)
export async function getAllReturns() {
  try {
    ...
    snapshot.docs.forEach(docSnap => {
      ...
      // ✅ FIX: No more dividing IQD by 1500!
