...
export async function searchPharmacies(searchQuery) {
  try {
    ...
    // If search query is empty or just whitespace, return all pharmacies
    if (!searchQuery || searchQuery.trim() === "") {
      const snapshot = await getDocs(pharmaciesRef);
      return snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
    }
    
    // Get all pharmacies and filter client-side for case-insensitive search
    const snapshot = await getDocs(pharmaciesRef);
    const allPharmacies = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
    
    const searchLower = searchQuery.toLowerCase().trim();
    
    // Filter pharmacies by name or code (case-insensitive)
    ...
    // Fallback: try to get all pharmacies
    try {
...
// ============================================================
// REPLACE THE ENTIRE searchInitializedItems FUNCTION IN data.js
// ============================================================
...
export async function searchInitializedItems(searchQuery, searchType = "both") {
  ...
  try {
    // 🔥 CRITICAL FIX: Force refresh store items to get real-time stock
    const storeItems = await getStoreItems(true); // true = force refresh
    
    const searchLower = searchQuery.trim().toLowerCase();
    const searchTerms = searchLower.split(/\s+/).filter(term => term.length > 0);
    
    // Filter store items that have stock
    ...
    // If no results in store items, fall back to master items (but mark them as out of stock)
    if (results.length === 0) {
      const itemsRef = collection(db, ITEMS_COLLECTION);
      const snapshot = await getDocs(itemsRef);
...
// ============================================================
// UPDATE getStoreItems FUNCTION IN data.js
// ============================================================
...
export async function getStoreItems(forceRefresh = false) {
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      // 🔥 CRITICAL: Only include items with quantity > 0
      // Items with 0 quantity should not appear in store
...
// ===================== COMPLETE createBoughtBill FUNCTION =====================
// Replace your existing createBoughtBill function with this one
...
export async function createBoughtBill(
  ...
  try {
    ...
    for (const item of billItems) {
      ...
      if (!catalogId) {
        try {
          ...
          if (existingCatalog) {
            // Use existing catalog ID
            catalogId = existingCatalog.id;
            console.log(`✅ Found existing catalog item: "${item.name}" (${catalogId})`);
          } else {
            // Create new catalog item (invisible by default)
            const newCatalogItem = await createCatalogItem({
              ...
              isVisible: false, // IMPORTANT: Invisible by default
              unit: item.unit || "piece",
              barcode: item.barcode || "",
            });
            catalogId = newCatalogItem.id;
            console.log(`🆕 Created new catalog item: "${item.name}" (${catalogId}) - INVISIBLE`);
          }
        } catch (catalogError) {
          console.error(`⚠️ Failed to sync item "${item.name}" to catalog:`, catalogError);
          // Continue anyway - don't block the bill creation
        }
      }

      // Build the store item with catalog reference
      const storeItem = {
        barcode: item.barcode,
        name: item.name,
    ...
    // ==================== CREATE NEW BILL ====================
    // Auto-create catalog items BEFORE adding to store
    // (This is already done in the items processing loop above)
...
export async function createSoldBill(billData) {
  try {
    ...
    // This is the CRITICAL fix - deduct before creating the bill
    ...
    for (const item of preparedItems) {
      ...
      // If a specific batch was selected, try to use it first
      if (item.batchId) {
        try {
          ...
          if (exactSnap.exists()) {
            ...
            if (data.barcode === item.barcode && data.quantity > 0) {
              ...
              console.log(`✅ Deducted ${deductQty} from exact batch ${exactSnap.id} for ${item.name}`);
            }
          }
        } catch (e) {
      ...
      // If still need more, find other batches (FIFO by expiry date)
      if (remainingQty > 0) {
        ...
        const spillQuery = query(
          ...
          where("branch", "==", branch),
          where("quantity", ">", 0) // Only get items with stock
        );
        ...
        for (const storeItem of candidates) {
          ...
          remainingQty -= deductQty;
          console.log(`✅ Deducted ${deductQty} from batch ${storeItem.id} for ${item.name}`);
        }
      }

      if (remainingQty > 0) {
        // Rollback all deductions for this item
    ...
    // ── STEP 2: Build the bill's item records ──────────────────────────
    const processedItems = preparedItems.map((item, idx) => {
      ...
      // Get the actual store item details for the batch used
      let selectedBatch = null;
      const allocations = allocationsByItem[idx] || [];
      if (allocations.length > 0) {
        // Use the first allocation's batch info
        const firstAlloc = allocations[0];
        // We already have the batch info from the deduction
      }
      ...
      return {
        ...
        // Store the EXACT allocations for this bill
    ...
    console.log(`📊 Stock deductions: ${allocationsByItem.flat().length} batches affected`);
    
    // Clear the store items cache
...
export async function updateBoughtBill(billNumber, updates) {
  try {
    // Don't throw error if updating attachment only - allow updating without items
    // Check if this is an attachment-only update
    const isAttachmentOnly = updates.attachment !== undefined && !updates.items;
    
    // Only require items if this is a full bill update
    ...
    // If this is attachment-only update, just update attachment fields
    if (isAttachmentOnly) {
      const updateData = {
    ...
    // Continue with full bill update...
    // Preserve original currency/exchange rate unless explicitly changed
    ...
    // ---------- Build the normalized "new" items list ----------
    ...
    // ---------- Diff against existing store item batches for this bill ----------
    ...
    for (const newItem of processedItems) {
      ...
      if (!existing) {
        // New line item added during this edit -> brand new batch
        await addDoc(collection(db, STORE_ITEMS_COLLECTION), {
          ...newItem,
      ...
      // Existing batch -> preserve however much has already been sold
    ...
    // Items that were REMOVED from the bill during this edit
    ...
    // ---------- Update the bill document itself ----------
    ...
    // Clear the in-memory store items cache so the UI reflects changes immediately
    storeItemsCache = null;
    lastFetchTime = 0;
...
export async function updateSoldBill(billNumber, updates) {
  try {
    ...
    // ─── STEP 1: RESTORE ORIGINAL QUANTITIES ───────────────────────────
    // Restore exactly what was deducted when the bill was created
    for (const originalItem of originalData.items || []) {
      const allocations = originalItem.batchAllocations || [];
      
      // Only restore if the item still exists in the updated items
      const stillExists = updates.items.some(item => 
      ...
      if (!stillExists) {
        // Item was removed, restore ALL quantity
        for (const alloc of allocations) {
          try {
            ...
            if (storeSnap.exists()) {
              await updateDoc(storeRef, {
                quantity: (storeSnap.data().quantity || 0) + alloc.quantity,
                updatedAt: serverTimestamp(),
              });
      ...
      const updatedItem = updates.items.find(item => 
        item.barcode === originalItem.barcode && 
        item.originalCurrency === originalItem.originalCurrency &&
        item.branch === originalItem.branch
      );
      ...
      if (diff > 0) {
        ...
        for (const alloc of allocations) {
          ...
          try {
            ...
            if (storeSnap.exists()) {
              await updateDoc(storeRef, {
                quantity: (storeSnap.data().quantity || 0) + restoreQty,
                updatedAt: serverTimestamp(),
              });
              remainingToRestore -= restoreQty;
              console.log(`✅ Restored ${restoreQty} to ${alloc.storeItemId}`);
            }
          } catch (e) {
            console.error(`Failed to restore ${alloc.storeItemId}:`, e);
          }
        ...
        // If there's still more to restore, find other batches
        if (remainingToRestore > 0) {
          ...
          for (const docSnap of findSnapshot.docs) {
            if (remainingToRestore <= 0) break;
            const data = docSnap.data();
            // Don't restore to a batch that already has allocations from this bill
            const alreadyAllocated = allocations.some(a => a.storeItemId === docSnap.id);
            if (alreadyAllocated) continue;
            
            const addQty = Math.min(remainingToRestore, data.quantity || 0);
            if (addQty > 0) {
              await updateDoc(doc(db, STORE_ITEMS_COLLECTION, docSnap.id), {
                quantity: (data.quantity || 0) + addQty,
                updatedAt: serverTimestamp(),
              });
              remainingToRestore -= addQty;
            }
          }
        }
      } else if (diff < 0) {
        ...
        // First try to use the original batches
        for (const alloc of allocations) {
          ...
          try {
            ...
            const storeSnap = await getDoc(storeRef);
            if (storeSnap.exists()) {
              const currentQty = storeSnap.data().quantity || 0;
              const deductQty = Math.min(remainingToDeduct, currentQty);
              if (deductQty > 0) {
                await updateDoc(storeRef, {
                ...
                console.log(`✅ Deducted ${deductQty} from ${alloc.storeItemId}`);
              }
            }
          } catch (e) {
            console.error(`Failed to deduct from ${alloc.storeItemId}:`, e);
          }
        }

        // If still need more, find other batches
        if (remainingToDeduct > 0) {
          ...
          for (const candidate of candidates) {
            if (remainingToDeduct <= 0) break;
            // Skip batches already allocated
            const alreadyAllocated = allocations.some(a => a.storeItemId === candidate.id);
            if (alreadyAllocated) continue;
    ...
    // ─── STEP 2: PREPARE NEW ALLOCATIONS FOR UPDATED ITEMS ─────────────
    const newAllocationsByItem = [];
    for (const newItem of updates.items) {
      const quantityNeeded = parseInt(newItem.quantity) || 1;
      if (quantityNeeded <= 0) {
        newAllocationsByItem.push([]);
        continue;
      }
      ...
      for (const candidate of candidates) {
        ...
        const deductQty = Math.min(remaining, candidate.quantity);
        allocations.push({ 
      ...
      if (remaining > 0) {
        throw new Error(`Insufficient stock for ${newItem.name}. Needed: ${quantityNeeded}, Short: ${remaining}`);
      }
      
      newAllocationsByItem.push(allocations);
    }

    // ─── STEP 3: UPDATE THE BILL ────────────────────────────────────────
    const { date: _ignoredDate, ...safeUpdates } = updates;
    if (safeUpdates.items && Array.isArray(safeUpdates.items)) {
      safeUpdates.items = safeUpdates.items.map((item, idx) => ({
        ...item,
        batchAllocations: newAllocationsByItem[idx] || [],
      }));
    }
      ...
      try {
        ...
        const validReturnItems = bill.items
          ...
          .map((item) => {
            ...
            // ✅ CRITICAL: Find store items from this specific bill
            const storeItem = storeItems.find(si => 
              String(si.barcode) === String(item.barcode) &&
              String(si.boughtBillNumber) === String(bill.billNumber)
            );
            
            // ✅ Available = what's currently in store from this bill
            ...
            console.log(`📊 ${item.name}: In Store: ${availableQuantity}, Original: ${item.quantity}, Returned: ${previouslyReturned}`);
            ...
            return {
              ...
              availableQuantity: Math.max(0, availableQuantity), // ✅ Only what's in store from this bill
              ...
              branch: item.branch || "Slemany",
              boughtBillNumber: bill.billNumber,
            };
          });
...
// ===================== NEW: createBoughtReturn =====================
// Creates a bought-return document (return to supplier) and deducts the
// returned quantities from the matching store batches for that bill.
export async function createBoughtReturn(companyId, items, note, returnBillNumber) {
  try {
    if (!companyId) throw new Error("Company ID is required");
    if (!items || items.length === 0) throw new Error("At least one item is required");

    const finalReturnBillNumber = returnBillNumber || (await generateBoughtReturnBillNumber());
    ...
    for (const item of items) {
      ...
      // Find store batches tied to this exact bought bill first (most accurate)
      ...
      let storeSnapshot = await getDocs(storeQuery);
      let candidates = storeSnapshot.docs.map((d) => ({ id: d.id, ...d.data(), quantity: Number(d.data().quantity) || 0 }));

      // Fallback: barcode + branch + currency, if nothing matched the exact bill
      if (candidates.length === 0) {
        ...
        storeSnapshot = await getDocs(storeQuery);
        candidates = storeSnapshot.docs.map((d) => ({ id: d.id, ...d.data(), quantity: Number(d.data().quantity) || 0 }));
      }

      const totalAvailable = candidates.reduce((sum, c) => sum + c.quantity, 0);
      if (totalAvailable < returnQty) {
        throw new Error(
          `Cannot return ${returnQty} of "${item.name}". Only ${totalAvailable} currently in store.`
        );
      }

      // Deduct FIFO by expiry date
      const sorted = candidates.slice().sort((a, b) => {
        const dA = a.expireDate?.toDate?.() || new Date(0);
        const dB = b.expireDate?.toDate?.() || new Date(0);
        return dA - dB;
      });
      ...
      for (const sd of sorted) {
        if (remaining <= 0) break;
        const deduct = Math.min(remaining, sd.quantity);
        const newQty = sd.quantity - deduct;
        if (newQty <= 0) {
          await deleteDoc(doc(db, STORE_ITEMS_COLLECTION, sd.id));
        } else {
        ...
        restoreAllocations.push({ storeItemId: sd.id, quantity: deduct });
        remaining -= deduct;
      }
      ...
      processedItems.push({
        ...
        restoredAllocations: restoreAllocations, // used later by deleteBoughtReturnItem / edit flows
      });
    }

    if (processedItems.length === 0) {
      throw new Error("No valid items with return quantity greater than 0");
    }

    const totalReturnQty = processedItems.reduce((s, it) => s + (Number(it.returnQuantity) || 0), 0);
    const totalReturnAmount = processedItems.reduce((s, it) => {
      const p = it.currency === "IQD" ? (it.returnPriceIQD || it.returnPrice || 0) : (it.returnPriceUSD || it.returnPrice || 0);
      return s + p * (Number(it.returnQuantity) || 0);
    }, 0);

    const returnDoc = {
      companyId,
      companyName: items[0]?.companyName || "",
      returnBillNumber: finalReturnBillNumber,
    ...
    const docRef = await addDoc(collection(db, BOUGHT_RETURNS_COLLECTION), returnDoc);
    ...
    console.error("Error creating bought return:", error);
    throw error;
  }
}

// ===================== NEW: deleteBoughtReturn =====================
// Deletes an entire bought-return document and restores all its items'
// quantities back into the exact store batches they were deducted from.
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (allocations.length > 0) {
        ...
        // Legacy fallback with no allocation tracking
        const storeQ = query(
          collection(db, STORE_ITEMS_COLLECTION),
          where("barcode", "==", item.barcode),
...
// ===================== NEW: updateBoughtReturnBill =====================
// Updates an ENTIRE return bill document (items array) in one atomic pass.
// Replaces the old per-item loop, which never wrote to the real `items[]` field.
export async function updateBoughtReturnBill(returnDocId, updatedItems, note) {
  try {
    if (!returnDocId) throw new Error("Return document ID is required");
    if (!updatedItems || updatedItems.length === 0) throw new Error("At least one item is required");

    const returnRef = doc(db, BOUGHT_RETURNS_COLLECTION, returnDocId);
    const returnSnap = await getDoc(returnRef);
    if (!returnSnap.exists()) throw new Error(`Return bill ${returnDocId} not found`);
    const returnDocData = returnSnap.data();

    // Normalize old flat-format docs into a 1-item array so both formats share one path
    const oldItems =
      Array.isArray(returnDocData.items) && returnDocData.items.length > 0
        ? returnDocData.items
        : returnDocData.barcode
        ? [{
            barcode: returnDocData.barcode,
            name: returnDocData.name,
            returnQuantity: returnDocData.returnQuantity || 0,
            billNumber: returnDocData.billNumber || "",
            branch: returnDocData.branch || "Slemany",
            currency: returnDocData.currency || "USD",
          }]
        : [];

    const billNumber = returnDocData.billNumber || oldItems[0]?.billNumber || "";

    // ── STEP 1: For every item in the NEW array, apply only the DELTA to store ──
    for (const newItem of updatedItems) {
      const barcode = String(newItem.barcode);
      const oldMatch = oldItems.find((oi) => String(oi.barcode) === barcode);
      const oldQty = Number(oldMatch?.returnQuantity) || 0;
      const newQty = Number(newItem.returnQuantity) || 0;
      const delta = newQty - oldQty;
      if (delta === 0) continue;

      const currency = newItem.currency || oldMatch?.currency || returnDocData.currency || "USD";
      const branch = newItem.branch || oldMatch?.branch || returnDocData.branch || "Slemany";
      const itemBillNumber = newItem.billNumber || billNumber;

      // Ground truth: how much was purchased
      let originalBillQuantity = Number(newItem.quantity) || Number(oldMatch?.quantity) || 0;
      try {
        const billData = await getBoughtBillByNumber(itemBillNumber);
        const billItem = billData?.items?.find((bi) => String(bi.barcode) === barcode);
        if (billItem) originalBillQuantity = Number(billItem.quantity) || originalBillQuantity;
      } catch (e) {}

      // How much is returned on OTHER return bills (never touch/trust this doc's old data twice)
      const otherDocsSnap = await getDocs(
        query(collection(db, BOUGHT_RETURNS_COLLECTION), where("billNumber", "==", itemBillNumber))
      );
      let returnedElsewhere = 0;
      otherDocsSnap.docs.forEach((d) => {
        if (d.id === returnDocId) return; // this bill is already represented by oldQty
        const data = d.data();
        if (Array.isArray(data.items)) {
          data.items.forEach((it) => {
            if (String(it.barcode) === barcode) returnedElsewhere += Number(it.returnQuantity) || 0;
          });
        } else if (String(data.barcode) === barcode) {
          returnedElsewhere += Number(data.returnQuantity) || 0;
        }
      });

      const maxReturnable = originalBillQuantity - returnedElsewhere;
      if (newQty > maxReturnable) {
        throw new Error(
          `Cannot set return quantity to ${newQty} for "${newItem.name}". Only ${maxReturnable} units remain ` +
          `eligible from bill #${itemBillNumber} (already returned on other bills: ${returnedElsewhere}).`
        );
      }
      ...
      const storeSnapshot = await getDocs(storeQ);
      const storeDocs = storeSnapshot.docs.map((d) => ({ id: d.id, ...d.data(), quantity: Number(d.data().quantity) || 0 }));

      if (delta > 0) {
        // Returning MORE than before -> deduct the extra from store
        const totalAvailable = storeDocs.reduce((s, d) => s + d.quantity, 0);
        if (totalAvailable < delta) {
          throw new Error(
            `Cannot increase return quantity of "${newItem.name}" by ${delta}. Only ${totalAvailable} units ` +
            `currently in store (some may already be sold).`
          );
        }
        const sorted = storeDocs.slice().sort((a, b) => {
          const dA = a.expireDate?.toDate?.() || new Date(0);
          const dB = b.expireDate?.toDate?.() || new Date(0);
          return dA - dB;
        });
        ...
        for (const sd of sorted) {
          if (remaining <= 0) break;
          const deduct = Math.min(remaining, sd.quantity);
          const newStoreQty = sd.quantity - deduct;
          if (newStoreQty <= 0) await deleteDoc(doc(db, STORE_ITEMS_COLLECTION, sd.id));
          else await updateDoc(doc(db, STORE_ITEMS_COLLECTION, sd.id), { quantity: newStoreQty, updatedAt: serverTimestamp() });
          remaining -= deduct;
        }
      } else {
        // Returning LESS than before -> add the difference back
        const addBack = Math.abs(delta);
        if (storeDocs.length > 0) {
          await updateDoc(doc(db, STORE_ITEMS_COLLECTION, storeDocs[0].id), {
            quantity: storeDocs[0].quantity + addBack,
            updatedAt: serverTimestamp(),
          });
        } else {
          await addDoc(collection(db, STORE_ITEMS_COLLECTION), {
            ...
            name: newItem.name,
            quantity: addBack,
            outPriceUSD: newItem.outPriceUSD || 0,
            outPriceIQD: newItem.outPriceIQD || 0,
            netPriceUSD: newItem.netPriceUSD || 0,
            netPriceIQD: newItem.netPriceIQD || 0,
            basePriceUSD: newItem.basePriceUSD || 0,
            basePriceIQD: newItem.basePriceIQD || 0,
            expireDate: newItem.expireDate || null,
            isConsignment: newItem.isConsignment || false,
            consignmentOwnerId: newItem.consignmentOwnerId || null,
            originalCurrency: currency,
            branch,
            boughtBillNumber: itemBillNumber,
            createdAt: serverTimestamp(),
            updatedAt: serverTimestamp(),
          });
        }
    ...
    // ── STEP 2: Items that existed before but were removed entirely -> restore their FULL quantity ──
    for (const oldItem of oldItems) {
      const stillExists = updatedItems.some((ni) => String(ni.barcode) === String(oldItem.barcode));
      if (stillExists) continue;
      const restoreQty = Number(oldItem.returnQuantity) || 0;
      if (restoreQty <= 0) continue;
      const currency = oldItem.currency || returnDocData.currency || "USD";
      const branch = oldItem.branch || returnDocData.branch || "Slemany";
      const storeQ = query(
        collection(db, STORE_ITEMS_COLLECTION),
        where("barcode", "==", oldItem.barcode),
        where("originalCurrency", "==", currency),
        where("branch", "==", branch)
      );
      const storeSnapshot = await getDocs(storeQ);
    ...
    // ── STEP 3: Write the new items array + recomputed totals back — this is what the UI reads ──
    const totalReturnQty = updatedItems.reduce((s, it) => s + (Number(it.returnQuantity) || 0), 0);
    const totalReturnAmount = updatedItems.reduce((s, it) => {
      const p = it.currency === "IQD" ? (it.returnPriceIQD || it.returnPrice || 0) : (it.returnPriceUSD || it.returnPrice || 0);
      return s + p * (Number(it.returnQuantity) || 0);
    }, 0);
    ...
    await updateDoc(returnRef, {
      items: updatedItems,
      returnNote: note !== undefined ? String(note) : returnDocData.returnNote || "",
      totalReturnQty,
      totalReturnAmount,
      updatedAt: serverTimestamp(),
    });
    ...
    return { id: returnDocId, items: updatedItems, totalReturnQty, totalReturnAmount, message: "Return bill updated successfully" };
  } catch (error) {
    console.error("Error updating bought return bill:", error);
    throw error;
  }
}


export async function deleteBoughtReturnItem(returnDocId, barcode) {
  try {
    if (!returnDocId) throw new Error("Return document ID is required");
    ...
    const itemsArray =
      Array.isArray(returnDocData.items) && returnDocData.items.length > 0
        ...
        ? [{
            ...
            branch: returnDocData.branch || "Slemany",
            currency: returnDocData.currency || "USD",
            restoredAllocations: returnDocData.restoredAllocations || [],
          }]
        : [];

    const itemIndex = itemsArray.findIndex((it) => String(it.barcode) === targetBarcode);
    if (itemIndex === -1) throw new Error(`Item with barcode ${targetBarcode} not found in this return bill`);

    const item = itemsArray[itemIndex];
    const restoreQty = Number(item.returnQuantity) || 0;
    const restoredAllocations = item.restoredAllocations || [];

    // ── STEP 1: Restore quantity to store using the exact allocations ──
    if (restoreQty > 0 && restoredAllocations.length > 0) {
      // Restore using the exact allocations from when the return was created
      for (const alloc of restoredAllocations) {
        try {
          const storeRef = doc(db, STORE_ITEMS_COLLECTION, alloc.storeItemId);
          const storeSnap = await getDoc(storeRef);
          if (storeSnap.exists()) {
            ...
            // The store item was deleted, recreate it
            // Find the original item data from the return
            const originalItem = itemsArray[itemIndex];
            await addDoc(collection(db, STORE_ITEMS_COLLECTION), {
              barcode: originalItem.barcode,
              name: originalItem.name,
              quantity: Number(alloc.quantity),
              netPriceUSD: originalItem.currency === "USD" ? originalItem.returnPrice : 0,
              netPriceIQD: originalItem.currency === "IQD" ? originalItem.returnPrice : 0,
              outPriceUSD: originalItem.currency === "USD" ? originalItem.returnPrice * 1.5 : 0,
              outPriceIQD: originalItem.currency === "IQD" ? originalItem.returnPrice * 1.5 : 0,
              expireDate: originalItem.expireDate || null,
              branch: originalItem.branch || "Slemany",
              isConsignment: originalItem.isConsignment || false,
              consignmentOwnerId: originalItem.consignmentOwnerId || null,
              originalCurrency: originalItem.currency || "USD",
              boughtBillNumber: originalItem.billNumber || null,
              createdAt: serverTimestamp(),
              updatedAt: serverTimestamp(),
            });
          }
      ...
      // Fallback: no allocations tracking, try to find a matching store item
      const currency = item.currency || returnDocData.currency || "USD";
      const branch = item.branch || returnDocData.branch || "Slemany";
      ...
      if (!storeSnapshot.empty) {
        ...
        await addDoc(collection(db, STORE_ITEMS_COLLECTION), {
          ...
          name: item.name,
          quantity: restoreQty,
          netPriceUSD: currency === "USD" ? item.returnPrice : 0,
          netPriceIQD: currency === "IQD" ? item.returnPrice : 0,
          outPriceUSD: currency === "USD" ? item.returnPrice * 1.5 : 0,
          outPriceIQD: currency === "IQD" ? item.returnPrice * 1.5 : 0,
          expireDate: item.expireDate || null,
          branch: branch,
          isConsignment: item.isConsignment || false,
          consignmentOwnerId: item.consignmentOwnerId || null,
    ...
    // ── STEP 2: Remove this item from the array ──
    itemsArray.splice(itemIndex, 1);

    // ── STEP 3: Update or delete the return document ──
    if (itemsArray.length === 0) {
      await deleteDoc(returnRef);
    } else {
      const totalReturnQty = itemsArray.reduce((s, it) => s + (Number(it.returnQuantity) || 0), 0);
      const totalReturnAmount = itemsArray.reduce((s, it) => {
        const p = it.currency === "IQD" ? (it.returnPriceIQD || it.returnPrice || 0) : (it.returnPriceUSD || it.returnPrice || 0);
        return s + p * (Number(it.returnQuantity) || 0);
      }, 0);
      await updateDoc(returnRef, {
    ...
    return {
      success: true,
      itemsRestored: restoreQty,
      billDeleted: itemsArray.length === 0,
    };
  } catch (error) {
...
// ===================== NEW: generateBoughtReturnBillNumber =====================
// Format: BRET-YYYYMMDD-N, sequential per day, starting from 1
export async function generateBoughtReturnBillNumber() {
  try {
    const now = new Date();
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, "0");
    const day = String(now.getDate()).padStart(2, "0");
    const datePrefix = `BRET-${year}${month}${day}-`;

    const snapshot = await getDocs(collection(db, BOUGHT_RETURNS_COLLECTION));
    let maxSeq = 0;
    snapshot.docs.forEach((d) => {
      const rbn = d.data().returnBillNumber;
      if (typeof rbn === "string" && rbn.startsWith(datePrefix)) {
        const seq = parseInt(rbn.slice(datePrefix.length), 10);
...
// In data.js - COMPLETE FIXED getBoughtBillByNumber function
export async function getBoughtBillByNumber(billNumber) {
  try {
    ...
    console.log(`🔍 Looking for bought bill: ${billNumber} (${typeof billNumber})`);
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (!snapshot.empty) {
          ...
          console.log(`✅ Found bill using ${strategy.field}: ${strategy.value}`);
          break;
        }
      } catch (err) {
    ...
    // If not found, try searching all bills
    if (!billData) {
      console.log(`🔍 Searching all bills for ${billNumber}...`);
      const allBillsSnapshot = await getDocs(billsRef);
      
      for (const doc of allBillsSnapshot.docs) {
        const data = doc.data();
        
        // Check if billNumber matches
        if (String(data.billNumber) === String(billNumber) ||
            Number(data.billNumber) === Number(billNumber)) {
          billData = {
            id: doc.id,
            ...data,
            date: data.date?.toDate ? data.date.toDate() : data.date,
          ...
          console.log(`✅ Found bill by scanning: ${billNumber}`);
          break;
        }
        
        // Check if any item has this billNumber
        if (data.items && Array.isArray(data.items)) {
          ...
          if (matchingItem) {
            ...
            console.log(`✅ Found bill via item search: ${billNumber}`);
            break;
          }
        }
      }
    ...
    if (!billData) {
      console.warn(`⚠️ Bill ${billNumber} not found in any query`);
      
      // Return a minimal bill object with the items from the return
      // This allows the return to proceed even if the bill is not found
      return {
        id: null,
        billNumber: billNumber,
        items: [],
...
// Sync store items to catalog (when a new item is bought)
export async function syncStoreItemToCatalog(storeItem) {
  try {
    if (!storeItem || !storeItem.name) return null;
    
    // Check if item exists in catalog by name
    const existing = await getCatalogItemByName(storeItem.name);
    if (existing) {
      // Update base price if needed
      if (storeItem.netPrice && storeItem.netPrice !== existing.basePrice) {
        // Optionally update base price or keep as is
        console.log(`Item "${storeItem.name}" already exists in catalog. Price: ${existing.basePrice}`);
      }
      return existing;
    }
    ...
    console.log(`Created new catalog item "${storeItem.name}" (invisible)`);
    return newItem;
  } catch (error) {
...
// ===================== CATALOG MANAGEMENT FUNCTIONS =====================
// Add these to your data.js

const CATALOG_COLLECTION = "catalogItems";

// Get all catalog items with stock info
...
// Get catalog item by name (case insensitive)
...
export async function updateCatalogItem(itemId, itemData) {
  try {
    ...
    // Check if updating name and it conflicts with another item
    if (itemData.name) {
      const existing = await getCatalogItemByName(itemData.name);
...
export async function syncStoreItemsToCatalog() {
  try {
    ...
    // Create a map of existing catalog items by name (lowercase)
    const catalogMap = {};
    ...
    for (const storeItem of storeItems) {
      try {
        ...
        // Get the price - use outPrice or netPrice
        let basePrice = storeItem.outPrice || storeItem.netPrice || 0;
        // If both USD and IQD are available, use USD
        if (storeItem.outPriceUSD && storeItem.outPriceUSD > 0) {
          ...
          basePrice = storeItem.outPriceIQD / 1500; // Convert IQD to USD roughly
        }
        
        // Create catalog item (invisible by default)
        ...
        // Add to map to avoid duplicates in same sync
...
// Get all catalog items (without stock info)
...
export async function getReturnsForPharmacy(pharmacyId) {
  try {
    ...
    // Filter: Only return UNPAID returns (not Processed or Paid)
    ...
    console.log(`Found ${unpaidReturns.length} unpaid returns for pharmacy ${pharmacyId}`);
...
// ── B) getReturnById: carry batch info through into the flat items list ────
// Inside getReturnById, in the loop that pushes onto `items`, add the two
// NEW lines below (everything else in that push stays the same):
//
//   items.push({
//     id:              docSnap.id,
//     barcode:         item.barcode,
...
// ── C) Helper shared by create/update: distribute a return quantity across
//       the original sale's batchAllocations, skipping past what earlier
//       returns already consumed, and apply it directly to those exact
//       storeItemIds. Falls back to barcode+branch+currency matching only
//       when no batch allocation data exists (legacy sales). ─────────────
async function r9yMnTm4NSzvG9rrwjM2ec8xZgh1cafXH8({ barcode, name, branch, currency, expireDate, returnPrice, returnQty, saleBatchAllocations, skipQty }) {
  const applied = [];
  ...
  for (const alloc of (saleBatchAllocations || [])) {
    ...
    try {
      ...
      // If the exact batch doc no longer exists, fall through — it'll be
      // covered by the fallback below instead of silently vanishing.
    } catch (e) {
      console.error(`Failed to restore to batch ${alloc.storeItemId}:`, e);
    }
  }

  // Fallback: no batch allocation data (legacy sale) or allocations didn't
  // cover the full return — match by barcode+branch+currency as a last resort.
  if (remaining > 0) {
    const storeItemsRef = collection(db, STORE_ITEMS_COLLECTION);
    const sq = query(storeItemsRef, where("barcode", "==", barcode), where("branch", "==", branch));
    const storeSnapshot = await getDocs(sq);
    const candidates = storeSnapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
    ...
    if (target) {
      ...
      const newDoc = await addDoc(collection(db, STORE_ITEMS_COLLECTION), {
        barcode, name: name || "Unknown Item", quantity: remaining,
        expireDate: expireDateTimestamp, branch,
        isConsignment: false, consignmentOwnerId: null,
        originalCurrency: currency, priceType: currency,
        ...
        basePriceUSD: 0, basePriceIQD: 0,
        boughtBillNumber: "RETURNED",
...
async function restoreQuantityToStore({ barcode, name, branch, currency, expireDate, returnPrice, returnQty, saleBatchAllocations, skipQty, boughtBillNumber }) {
  ...
  // ── Tier 1: exact batch restore using the sale's own batchAllocations ──────
  for (const alloc of (saleBatchAllocations || [])) {
    if (remaining <= 0) break;
    const allocQty = Number(alloc.quantity) || 0;
    if (skip >= allocQty) { skip -= allocQty; continue; }
  ...
  // ── Tier 2: barcode + branch + boughtBillNumber ─────────────────────────────
  ...
  // ── Tier 3: barcode + branch only (last resort) ─────────────────────────────
  if (remaining > 0) {
    const storeItemsRef = collection(db, STORE_ITEMS_COLLECTION);
    const sq = query(storeItemsRef, where("barcode", "==", barcode), where("branch", "==", branch));
    const storeSnapshot = await getDocs(sq);
  ...
  try {
    ...
    for (const item of items) {
      ...
      const applied = await restoreQuantityToStore({
        ...
        returnQty,
        saleBatchAllocations: item.saleBatchAllocations || [],
        skipQty: item.alreadyReturned || 0, // qty already restored by earlier returns for this bill+item
        boughtBillNumber: item.boughtBillNumber || null,
      });
      ...
      itemsWithAllocations.push({
        ...
        branch: item.branch || "Slemany",
        boughtBillNumber: item.boughtBillNumber || null,
        saleBatchAllocations: item.saleBatchAllocations || [], // keep for future edits
        restoreAllocations: applied,                            // exactly what THIS return touched
      });
    }
...
// ── E) deleteReturnBillAndRestoreToSale — reverse the exact batches touched ─
export async function deleteReturnBillAndRestoreToSale(returnId) {
  try {
    if (!returnId) throw new Error("Return ID is required");

    let returnRef, returnData, actualReturnId = returnId;
    ...
    const itemsToReverse =
      Array.isArray(returnData.items) && returnData.items.length > 0
        ? returnData.items
        : returnData.barcode ? [returnData] : [];

    for (const item of itemsToReverse) {
      const restoreAllocations = item.restoreAllocations || [];

      if (restoreAllocations.length > 0) {
        // Precise reversal: undo exactly what was added to these exact batches.
        for (const alloc of restoreAllocations) {
          try {
            const storeRef = doc(db, STORE_ITEMS_COLLECTION, alloc.storeItemId);
            const storeSnap = await getDoc(storeRef);
            if (storeSnap.exists()) {
            ...
            console.error(`Failed to reverse batch ${alloc.storeItemId}:`, e);
          }
        }
      } else {
        // Legacy return with no batch tracking — fall back to old behavior.
        const returnQty = Number(item.returnQuantity) || 0;
        if (returnQty <= 0) continue;
        const barcode = item.barcode;
        const currency = item.currency || returnData.currency || "IQD";
        const branch = item.branch || "Slemany";
        const sq = query(collection(db, STORE_ITEMS_COLLECTION), where("barcode", "==", barcode), where("branch", "==", branch));
        const storeSnapshot = await getDocs(sq);
        const candidates = storeSnapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
        const matchByCurrency = candidates.filter((c) => (c.originalCurrency || c.priceType) === currency);
        const target = matchByCurrency[0] || candidates[0];
        if (target) {
          const newQty = (Number(target.quantity) || 0) - returnQty;
          if (newQty <= 0) await deleteDoc(doc(db, STORE_ITEMS_COLLECTION, target.id));
          else await updateDoc(doc(db, STORE_ITEMS_COLLECTION, target.id), { quantity: newQty, updatedAt: serverTimestamp() });
        }
      }
    }
...
export async function updateReturnItems(returnBillNumber, items, totalAmount, totalQty) {
  try {
    ...
    for (const newItem of items) {
      ...
      // STEP 1 — reverse exactly what this return previously added.
      ...
      // STEP 2 — reapply for the new quantity, skipping past what OTHER
      // return bills (not this one) have already consumed.
    ...
    if (items.length > 0 && items[0].currency) updateData.currency = items[0].currency;
